...
AddOption('--enable-mysql', dest='enable-mysql', action='store_true',
            help='Enable mysql')
            
AddOption('--enable-gmalg', dest='enable-gmalg',action='store_true',
            help='Enable GM algorithm')
...
def add_boost_and_protobuf(toolchain, env):
    ...
    try:
        ...
        proto_lib_path = os.path.join(PROTOBUF_ROOT, 'src', '.libs')
        proto_include = os.path.join(PROTOBUF_ROOT, 'src')
        if not os.path.exists(proto_lib_path):
            proto_lib_path = os.path.join(PROTOBUF_ROOT, 'lib')
            proto_include = os.path.join(PROTOBUF_ROOT, 'include')
            
        env.Append(LIBPATH=[proto_lib_path])
        
        if not should_link_static() and toolchain in['clang', 'gcc']:
            env.Append(LINKFLAGS=['-Wl,-rpath,' + proto_lib_path])
        env['PROTOBUF_ROOT'] = PROTOBUF_ROOT
        env.Append(CPPPATH=[proto_include])
    except KeyError:
        pass
...
def enable_mysql():
    return GetOption('enable-mysql')

def enable_gmalg():
    return GetOption('enable-gmalg')
...
def config_env(toolchain, variant, env):
    ...
    if toolchain in Split('clang gcc'):
        ...
        env.Append(LIBS=['dl'])
        if enable_gmalg():
            env.Append(LIBPATH='./third_party/gmencryptlib/linux/')
            env.Append(LIBS='libswsds.so')
            env.Append(LIBS='libswsd.so')
            #env.Append(LIBS='libciphersuite_smengine.so')
            #env.Append(LIBS='libciphersuite_crypto.so')
            #env.Append(LIBS='libciphersuite_ssl.so')
            env.Append(LINKFLAGS=['-Wl,-rpath,' + './gmencryptlib/linux/'])
        ...
        if Beast.system.osx:
            ...
            static_libs = ['%s/lib/lib%s.a' % (env['BOOST_ROOT'], l) for
                           l in boost_libs]
